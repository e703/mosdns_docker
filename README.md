...
- 20260715版本调整内容如下:
  - 添加update-rules.sh文件用于升级分流文件；
  - 添加用于X64环境v2dat可执行文件用于生成分流文件；
  - 配置示例“&& echo "0 2 * * 0 /bin/sh /update-rules.sh >> /var/log/update-rules.log 2>&1" >> /var/spool/cron/crontabs/root”
